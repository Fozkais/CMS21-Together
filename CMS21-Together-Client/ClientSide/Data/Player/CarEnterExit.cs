...
public static class CarEnterExit
{
	...
	private static bool lastEnginePlaying;
	private static float lastEngineCheckTime;
	...
	public static void CheckCarState()
	{
		...
		var currentlyInCar = false;
		var currentCarLoaderID = -1;
		...
		if (mainCamera != null && mainCamera.transform != null && GameData.Instance.carLoaders != null)
		{
			...
			if (cameraParent != null)
				for (var i = 0; i < GameData.Instance.carLoaders.Length; i++)
				{
		...
		if (currentlyInCar && currentCarLoaderID >= 0) CheckEngineSound(currentCarLoaderID);
	}
