...
public static class SteamworksUtils
{
	...
	{
		var SV_client = Server.Instance.clients.First(s => s.Value.steam.connection.Id == connection.Id).Value;
		if (SV_client == null)
	...
	{
		var offset = Random.Next(0, 62);

		var result = new StringBuilder();
		do
		{
			var index = (int)(lobbyID % 62);

			var newChar = Characters[(index + offset) % 62];
	...
	{
		var offsetChar = code[code.Length - 1];
		var offset = Characters.IndexOf(offsetChar);

		ulong result = 0;
		for (var i = 0; i < code.Length - 1; i++)
		{
			var index = Characters.IndexOf(code[i]);

			var originalIndex = (index - offset + 62) % 62;
