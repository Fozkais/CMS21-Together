...
using MelonLoader;
using UnityEngine;
using SceneManager = CMS21Together.ClientSide.SceneManager;
...
public static class CarSpawnHooks
{
	...
	{
		if (!Client.Instance.isConnected || !listenToLoad) listenToLoad = true;

		var indexFromCarLoaderName = Helper.GetIndexFromCarLoaderName(file);
		var newCarData = Singleton<GameManager>.Instance.GameDataManager.LoadCar(indexFromCarLoaderName, false);
