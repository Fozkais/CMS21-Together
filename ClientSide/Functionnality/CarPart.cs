        ...
        public async static void HandleEngineParts()
        {
            ...
            foreach (var car in OriginalEnginePartsCopy)
            {
                if (CarSpawn.CarHandle[car.Key].FinishedPreHandlingEngine)
                {
                    ...
                    {
                        var part = MPGameManager
                            .OriginalEngineParts[car.Key]; // Toutes les pieces[i] ou i = carLoaderID
                        ...
                        foreach (var _part in part)
                        {
                            ...
                            if (MPGameManager.EnginePartsHandle.ContainsKey(car.Key))
                            {
                                ...
                                if (!MPGameManager.EnginePartsHandle[car.Key]
                                        .Any(s => s.Value._UniqueID ==
                                                  part[_part.Key]
                                                      ._UniqueID)) // Vérifie la non-préscense de la sous-piece[j] dans le handle
                                {
                                    //MelonLogger.Msg("New part Found ! sending new pieces");
                                    if (!MPGameManager.EnginePartsHandle[car.Key].ContainsKey(_part.Key))
                                    {
                                        MPGameManager.EnginePartsHandle[car.Key]
                                            .Add(_part.Key, part[_part.Key]._PartScriptInfo);
                                ...
                                if (MPGameManager.EnginePartsHandle[car.Key]
                                    .Any(s => s.Value._UniqueID ==
                                              part[_part.Key]
                                                  ._UniqueID)) // Vérifie la préscense de la sous-piece[j] dans le handle
                                {
                                    PartScriptInfo
                                        partInfo = MPGameManager
                                            .EnginePartsHandle[car.Key][
                                                _part.Key]; // sous piece a l'index [i][j][index]
                                    PartScriptData
                                        partToCompare =
                                            new PartScriptData(part[_part.Key]
                                                ._partScript); // sous-piece originalPart[i][j][k]
                                    if (HasDifferences(partToCompare, partInfo._partScriptData))
                                    {
                                        MelonLogger.Msg("Differences Found ! sending new pieces");
                                        partInfo._partScriptData = partToCompare;
        ...
        public async static void HandleSuspensionParts()
        {
            ...
            foreach (var car in OriginalSuspensionPartsCopy)
            {
                if (CarSpawn.CarHandle[car.Key].FinishedPreHandlingPart)
                {
                    ...
                    {
                        var part = MPGameManager
                            .OriginalSuspensionParts[car.Key]; // Toutes les pieces[i] ou i = carLoaderID
                        ...
                        foreach (var _part in part)
                        {
                            ...
                            for (int k = 0;
                                 k < part[_part.Key].Count;
                                 k++) // Parcours des sous-pieces pour i = carLoaderID, j = piece, k = sous-pieces
                            {
                                ...
                                if (!MPGameManager.SuspensionPartsHandle[car.Key][_part.Key]
                                        .Exists(s =>
                                            s._UniqueID ==
                                            part[_part.Key][k]
                                                ._UniqueID)) // Vérifie la non-préscense de la sous-piece[k] dans le handle
                                {
                                    //MelonLogger.Msg("New part Found ! sending new pieces");
                                ...
                                if (MPGameManager.SuspensionPartsHandle[car.Key][_part.Key]
                                    .Exists(s =>
                                        s._UniqueID ==
                                        part[_part.Key][k]
                                            ._UniqueID)) // Vérifie la préscense de la sous-piece[j][k] dans le handle
                                {
                                    ...
                                    PartScriptInfo partInfo =
                                        MPGameManager
                                            .SuspensionPartsHandle[car.Key][_part.Key][
                                                index]; // sous piece a l'index [i][j][index]
                                    PartScriptData
                                        partToCompare =
                                            new PartScriptData(part[_part.Key][k]
                                                ._partScript); // sous-piece originalPart[i][j][k]
                                    if (HasDifferences(partToCompare, partInfo._partScriptData))
                                    {
                                        MelonLogger.Msg("Differences Found ! sending new pieces");
