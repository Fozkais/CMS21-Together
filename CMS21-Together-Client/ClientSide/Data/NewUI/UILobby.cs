...
public static class UILobby
{
	public static void CreateLobby(bool isHost, string serverID, int save_index = -1)
	{
		...
		UICustomPanel.CreateSplitter(UICore.TMP_Window.transform, new Vector2(0, -40), new Vector2(440, 2));

		var saveName = isHost ? SavesManager.ModSaves[save_index].Name : "Game";
		...
		UICustomPanel.CreateSplitter(UICore.TMP_Window.transform, new Vector2(0, -75), new Vector2(440, 2));
	...
	{
		var pos = 10 - (index - 1) * 59;

		var player = new GameObject("PlayerPanel");
		...
		var content = username == "Waiting for player..." ? "" : "Not Ready";
		...
		UICustomPanel.CreateSplitter(player.transform, new Vector2(0, -20), new Vector2(440, 2));
		return player;
	}
	...
	public static void RefreshPlayers()
	{
		...
		foreach (var data in ClientData.Instance.connectedClients.Values)
		{
		...
		var ready_player = ClientData.Instance.connectedClients.Count(p => p.Value.isReady);
	...
	{
		var lobbyWindow = UICore.TMP_Window;

		for (var i = lobbyWindow.transform.childCount - 1; i >= 6; i--) Object.Destroy(lobbyWindow.transform.GetChild(i).gameObject);
	}
}