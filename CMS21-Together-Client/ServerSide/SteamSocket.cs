...
public class SteamSocket : SocketManager
{
	...
	public override void OnConnectionChanged(Connection connection, ConnectionInfo info)
	{
		...
		if (info.State == ConnectionState.Connecting)
		{
			...
			var isFull = true;
			foreach (var ClientID in Server.Instance.clients.Keys)
				if (!Server.Instance.clients[ClientID].isConnected)
					isFull = false;
	...
	{
		var _data = SteamworksUtils.ConvertIntPtrToByteArray(data, size);
